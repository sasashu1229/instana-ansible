...
.
├── README.md                           # このファイル
├── ansible-installv20260202.yaml       # 新規インストール用playbook
├── ansible-upgrade-20260202.yaml       # アップグレード用playbook
├── ansible_vars.yaml                   # 変数定義ファイル
├── inventory.ini                       # インベントリファイル（ホスト定義）
└── files/                              # 配布ファイル
    ├── instana-agent-static-j9.x86_64.rpm
    ├── configuration.yaml
...
```bash
ansible-playbook -i inventory.ini ansible-upgrade-20260202.yaml
```

### 3. 特定のホストのみ実行

```bash
ansible-playbook -i inventory.ini ansible-upgrade-20260202.yaml --limit "ホスト名"
```

### 4. ドライラン（変更を実際に適用せずに確認）

```bash
ansible-playbook -i inventory.ini ansible-upgrade-20260202.yaml --check
```
...
### ansible-installv20260202.yaml（新規インストール）
...
### ansible-upgrade-20260202.yaml（アップグレード）
...
- `ansible-installv20260202.yaml` は既にインストール済みの場合はスキップします
- アップグレードする場合は `ansible-upgrade-20260202.yaml` を使用してください
